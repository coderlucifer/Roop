...
function Navbar({ setOpenLeft, setOpenRight }) {
  ...
  return (
    <header
      className={`group fixed top-0 left-0 w-full px-4 py-3 sm:px-6 lg:px-12 lg:py-4 grid grid-cols-[1fr_1fr] lg:grid-cols-[1fr_auto_1fr] items-center z-[100] transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${
        scrolled
    ...
    >
      {/* LEFT */}
      <div className="flex items-center gap-3 sm:gap-6 shrink justify-start order-1 lg:order-none">
        <button
      ...
      {/* CENTER */}
      <div className="flex justify-center items-center min-w-[120px] sm:min-w-[200px] col-span-2 lg:col-span-1 order-[-1] lg:order-none mb-1 lg:mb-0">
        <Link to="/">
          <img
            src={logo}
            alt="Roop logo"
            className={`h-[35px] sm:h-[45px] w-auto max-w-[130px] sm:max-w-[160px] object-contain transition-filter duration-400 ease-in-out ${
      ...
      {/* RIGHT */}
      <div className="flex justify-end items-center gap-3 sm:gap-6 text-inherit order-2 lg:order-none">
        <span
        ...
        <IconHeart className="cursor-pointer opacity-85 transition-opacity duration-150 hover:opacity-100 hidden sm:block" size={22} />
        {isAuthenticated ? (
